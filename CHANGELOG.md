...
- **浅色模式下 AI 气泡文字在深色主题网页上几乎不可见**：气泡/输入框文字颜色此前为 `inherit`，直接继承宿主网页 body 的文字颜色。在深色主题网页（浅色文字）上打开面板时，浅灰文字叠加浅灰气泡，对比度仅 ~1.2:1，几乎看不清。现已将浅色主题的文字颜色固定为深色（`#1f2937`），面板内文字不再依赖宿主网页颜色

### 🔧 重构

- **界面 emoji 字符替换为自绘 SVG 图标**：新增 `js/icons.js`（feather 风格线性图标，`stroke="currentColor"` 随主题变色，与面板头部按钮风格统一），替换范围：
  - 快捷操作按钮：按动作类型匹配图标（总结/提炼/翻译/自定义）＋纯文字 label
  - 选中文本按钮、页面上下文栏、已加载系统气泡、错误气泡
  - 关闭/新建/发送按钮、选项页删除按钮与连接测试状态提示（✅/❌/⚠️）
  - i18n 10 语言清理 90 处 emoji 前缀（欢迎消息 👋 与 About 彩蛋 🐘 属文案内容，保留）

## [1.18.4] - 2026-08-03

### ✨ 新功能

- **快捷操作按钮支持思考模式设置**：每个快捷按钮（默认三件套及用户自定义按钮）新增「思考模式」下拉选项（跟随全局设置 / 关闭思考）。默认的总结、大纲、翻译三个按钮固定为「关闭思考」——这类工具型任务不需要深度推理，关闭后响应更快、更省 token，也不再有思维链抢占输出的问题；用户编辑过默认按钮（视为自定义）后自动恢复为「跟随全局设置」，仍可手动选择关闭。任意自定义按钮也可单独配置

### 🔧 重构

- **快捷按钮改为直接发送 prompt**：点击快捷按钮不再把 prompt 填入输入框再触发发送，改为直接发送——避免冲掉用户正在输入的内容；思考模式设置随请求透传到后端（`thinking: off` 时对 DeepSeek 显式发送 `thinking: { type: "disabled" }`，与标题生成场景同一机制）
...
- **快捷按钮拖拽排序** — 选项页「快捷按钮」列表支持拖拽调整顺序：每张卡片左侧新增 ⠿ 拖拽把手（悬停高亮），按住把手上下拖动实时让位重排，松手自动保存到 `quickActions`，侧边栏快捷按钮顺序同步更新；拖拽到列表外释放视为取消，自动恢复原顺序，避免界面与存储不一致。新增 `dragSortHint` i18n key × 10 语言；E2E 新增「拖拽排序并持久化」「拖拽到列表外取消恢复」两个用例

## [1.17.2] - 2026-08-02

### 🐛 修复

- **空对话时导出/清除上下文/新建按钮无反馈**：此前对话未开始时点击导出、清除上下文、+ 新建按钮均无任何反应（内部静默返回），容易让用户误以为功能失灵。现已改为空对话（无消息）时三个按钮直接置灰禁用（`disabled` + 半透明样式），从视觉上明确不可用；发出第一条消息后恢复可用，删光消息后重新置灰。E2E 新增覆盖用例（空对话置灰 → disabled 不弹菜单 → 发消息恢复 → 删空重灰）
