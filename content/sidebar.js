...
function handleClickOutside(e) {
  ...
  if (!panel || !btn) return;
  // 用 composedPath 判断命中：按钮 click handler 内替换 innerHTML（如全屏/暗色图标切换）
  // 会使 e.target 脱离 DOM，panel.contains(e.target) 失效、误判为点击外部关闭面板。
  // composedPath 是事件派发时的路径快照，不受 DOM 后续变化影响。
...
function createButton() {
  try {
    ...
    btn.id = "__dp-btn";
    btn.title = "DeepPage";
    // 内联样式兜底：防止页面全局 CSS（如 Fluent UI 等框架的 button 规则）覆盖尺寸/形状。
    // 注意：不写 display，由 CSS 控制（#__dp-btn flex / #__dp-btn.__dp-hidden none），
    // 否则内联 display:flex 会压过 __dp-hidden 的 display:none，面板打开时按钮无法隐藏。
    btn.style.cssText =
      ...
      "padding:0;margin:0;border-radius:50%;border:none;background:#4a6cf7;" +
      "color:white;align-items:center;justify-content:center;" +
      "box-sizing:content-box;z-index:2147483646;";
    btn.innerHTML =
