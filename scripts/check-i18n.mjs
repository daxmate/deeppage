...
function extractTranslations(src) {
  ...
  // 括号配对（跳过字符串与注释）
  let depth = 0, inStr = false, strCh = '', inLineComment = false, inBlockComment = false;
  let end = -1;
...
if (unused.length) console.log(`ℹ️ 未被代码引用的 key（${unused.length} 个，仅提示）: ${unused.slice(0, 20).join(', ')}${unused.length > 20 ? '...' : ''}`);
