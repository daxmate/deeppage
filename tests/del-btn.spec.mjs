...
test.describe('消息删除按钮', () => {
  ...
  test('每条消息有删除按钮，hover 显示，删除同步 storage', async ({ page, sw }) => {
    ...
    const storage1 = await sw.evaluate(() => new Promise(resolve => chrome.storage.local.get('deeppage_convs', r => resolve(r.deeppage_convs))));
    ...
    expect(await page.locator('#__dp-chat .__dp-msg').first().evaluate(el => el.classList.contains('__dp-assistant'))).toBe(true);

    const storage2 = await sw.evaluate(() => new Promise(resolve => chrome.storage.local.get('deeppage_convs', r => resolve(r.deeppage_convs))));
    ...
    const storage3 = await sw.evaluate(() => new Promise(resolve => chrome.storage.local.get('deeppage_convs', r => resolve(r.deeppage_convs))));
