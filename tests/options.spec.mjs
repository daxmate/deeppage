...
test.describe('Options 页', () => {
  test('提供商下拉框渲染完整', async ({ extensionId, page }) => {
    ...
    await page.locator('#apiProvider option').first().waitFor({ state: 'attached', timeout: 15000 });
  ...
  test('修改参数自动保存到 storage（含流式开关）', async ({ extensionId, page, sw }) => {
    ...
    await page.locator('#temperature').evaluate(el => { el.value = '0.3'; el.dispatchEvent(new Event('change', { bubbles: true })); });
    await streamToggle.evaluate(el => { el.checked = false; el.dispatchEvent(new Event('change', { bubbles: true })); });

    // 自动保存是异步的，轮询等 storage 生效
    await expect(async () => {
      const stored = await sw.evaluate(() => new Promise(resolve => chrome.storage.sync.get(['temperature', 'streamOutput'], resolve)));
