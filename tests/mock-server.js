...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    if (req.method === 'GET' && (req.url === '/' || req.url === '/test-page')) {
      ...
      res.end('<!DOCTYPE html><html><head><title>Mock Test Page</title></head><body><h1>DeepPage Mock Page</h1><p id="content">这是用于上下文提取的 mock 页面内容。</p><p>Some English text for selection tests.</p><p id="target">Hello SPA world, select this text</p><div id="app"></div></body></html>');
      return;
    }
    ...
    if (isStream) {
      ...
      for (const ch of content) {
        res.write(`data: ${JSON.stringify({ id: 'mock-stream', object: 'chat.completion.chunk', created: Date.now(), model: 'mock', choices: [{ index: 0, delta: { content: ch }, finish_reason: null }] })}\n\n`);
      }
      res.write(`data: ${JSON.stringify({ id: 'mock-stream', object: 'chat.completion.chunk', created: Date.now(), model: 'mock', choices: [{ index: 0, delta: {}, finish_reason: 'stop' }] })}\n\n`);
      res.write('data: [DONE]\n\n');
      res.end();
    } else {
      sendJson(res, 200, {
        id: 'mock-chat', object: 'chat.completion', created: Date.now(), model: 'mock',
