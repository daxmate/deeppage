...
test.describe('悬浮按钮', () => {
  test('拖拽后位置变化并写入 storage，刷新后恢复', async ({ page, sw }) => {
    ...
    expect(Math.round(after.x) === Math.round(before.x) && Math.round(after.y) === Math.round(before.y)).toBe(false);

    // storage 写入 btnPos
    const stored = await sw.evaluate(() => new Promise(resolve => chrome.storage.sync.get('btnPos', r => resolve(r.btnPos))));
