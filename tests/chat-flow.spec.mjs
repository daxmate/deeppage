...
test.describe("聊天核心流程", () => {
  ...
  test("发送消息 → 用户气泡 + AI 流式回复渲染", async ({ page, mock }) => {
    ...
    // 用户气泡
    await expect(page.locator("#__dp-chat .__dp-msg.__dp-user .__dp-bubble-content")).toContainText(
      "你好，介绍一下你自己"
    );
    // AI 气泡（流式结束后完整内容）
    await expect(
  ...
  test("非流式响应同样渲染（关闭流式开关）", async ({ page, mock, setupMockApi }) => {
    ...
    await expect(
      page.locator("#__dp-chat .__dp-msg.__dp-assistant:not([data-msg-type]) .__dp-bubble-content")
    ).toContainText("非流式回复内容。", { timeout: 15000 });
    const { requests } = await mock.requests();
    const chatReq = requests.find((r) => r.url.includes("/chat/completions"));
  ...
  test("API 错误 → 显示错误信息", async ({ page, mock }) => {
    ...
    await expect(
      page.locator("#__dp-chat .__dp-msg.__dp-assistant:not([data-msg-type]) .__dp-bubble-content")
    ).toContainText("Invalid API key", { timeout: 15000 });
  });
  ...
  test("连续多轮对话 → 历史消息保留", async ({ page, mock }) => {
    ...
    const userMsgs = (await page.locator("#__dp-chat .__dp-msg.__dp-user").allTextContents()).map(
      (s) => s.trim()
    );
