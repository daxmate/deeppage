...
test('长文本流式渲染节流且内容完整', async ({ page, setupMockApi, mock }) => {
  ...
  const longText = Array.from({ length: 400 }, (_, i) => `段落${i}：这是第 ${i} 段内容，包含一些文字。`).join('\n\n');
  ...
  await page.evaluate(() => {
    ...
    new MutationObserver(() => { window.__renderCount++; }).observe(el, { childList: true, subtree: true, characterData: true });
  });

  await page.fill('#__dp-input', '生成超长回复');
  await page.click('#__dp-send');
  // 等回复完成
  await expect(page.locator('#__dp-chat .__dp-msg.__dp-assistant:not([data-msg-type]) .__dp-bubble-content')).toContainText('段落399', { timeout: 30000 });

  const result = await page.evaluate(() => ({
    renderCount: window.__renderCount,
    textLength: document.querySelector('#__dp-chat .__dp-msg.__dp-assistant:not([data-msg-type]) .__dp-bubble-content')?.textContent?.length || 0,
  }));
